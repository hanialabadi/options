...
def compute_days_to_earnings(
    ...
    if earnings_date:
        ...
        if delta < 0:
            # Earnings date is in the past — stale data from yfinance or static calendar.
            # Return None so all gate logic treats earnings as unknown (non-blocking).
            # A negative value would silently mislead income-strategy gates.
            logger.debug(
                f"[Earnings] {ticker}: date {earnings_date.date()} is {abs(delta)}d past "
                f"(snapshot {snapshot_date.date()}) — returning None (stale)"
            )
            return None
        return delta
