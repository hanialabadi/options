...
# Observable modules - Absolute imports preferred
from core.management.cycle2.drift.compute_dte import compute_dte
from core.management.cycle2.drift.compute_breakeven import compute_breakeven
from core.management.cycle2.drift.compute_moneyness import compute_moneyness
# from core.management.cycle1.enrich.auto_enrich_iv import auto_enrich_iv_from_archive
# from core.management.cycle1.enrich.compute_iv_rank import compute_iv_rank
# from core.management.cycle1.enrich.compute_earnings_proximity import compute_earnings_proximity
# from core.management.cycle1.enrich.compute_trade_aggregates import compute_trade_aggregates
from core.management.cycle2.drift.compute_pnl_metrics import compute_pnl_metrics, aggregate_trade_pnl
from core.management.cycle2.drift.compute_pnl_attribution import compute_pnl_attribution, aggregate_trade_pnl_attribution, rehydrate_entry_data
...
def run_phase3_enrichment(df: pd.DataFrame, snapshot_ts: pd.Timestamp = None) -> pd.DataFrame:
    ...
    4. Trade aggregates (requires leg-level Greeks)
    5. Structural enrichments (breakeven, moneyness)
    6. P&L and Performance Metrics
    ...
    # 5. Trade-level aggregates (CRITICAL: must come after leg-level Greeks)
    # df = compute_trade_aggregates(df)
    
    # 6. Structural enrichments
    # RAG: Structural Authority. Breakeven and Moneyness are required for Decision Engine.
    df = compute_breakeven(df)
    df = compute_moneyness(df)
    
    # 7. P&L and Performance Metrics
    df = compute_pnl_metrics(df, snapshot_ts=reference_ts)
    df = aggregate_trade_pnl(df)
    
    # RAG: Attribution Deferred. 
    # P&L Attribution is moved to Cycle 2 (Measurement) in run_all.py 
    # to ensure current underlying prices are fully enriched before calculation.
