...
def compute_breakeven(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def compute_group_breakeven(group):
        ...
        # === Covered Call ===
            ...
            if abs(call_premium) < 1e-9:
                # Zero premium — stale/expired/data gap. Warn and skip rather than crash.
                logger.warning(
                    f"⚠️ Covered Call TradeID {trade_id} has zero premium ({call_premium}). "
                    f"Using stock basis as breakeven."
                )
                return pd.Series([stock_basis, np.nan, BREAKEVEN_TYPE_COVERED_CALL])
            if call_premium > 0:
                raise ValueError(
