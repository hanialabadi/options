...
class SchwabClient:
    ...
    def ensure_valid_token(self) -> None:
        ...
        missing_vars = []
        if not os.getenv("SCHWAB_APP_KEY"): missing_vars.append("SCHWAB_APP_KEY")
        if not os.getenv("SCHWAB_APP_SECRET"): missing_vars.append("SCHWAB_APP_SECRET")
        
        if missing_vars:
            error_msg = (
                f"❌ MISSING AUTH CONFIG: {', '.join(missing_vars)}\n"
                "   Expected in: .env file or shell environment\n"
                "   Fix: export SCHWAB_APP_KEY='your_key' && export SCHWAB_APP_SECRET='your_secret'\n"
            )
