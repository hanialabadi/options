...
def fetch_contracts(df: pd.DataFrame, client: SchwabClient, expiry_intent: str = 'ANY') -> pd.DataFrame:
    ...
    if api_calls > 0:
        ...
        logger.info("   API unavailable (auth or empty input)")
...
def fetch_and_select_contracts_schwab(evaluated_strategies_df: pd.DataFrame, 
    ...
    try:
        ...
        logger.warning(f"⚠️ Schwab authentication failed: {e}")
        logger.warning("⏭️ Skipping Step 9B (Contract Fetching) - marking as AUTH_BLOCKED")
        
        result_df = merged.copy()
        result_df['Contract_Status'] = 'AUTH_BLOCKED'
        result_df['Failure_Reason'] = f"Authentication failed: {str(e)}"
        
        # Initialize required columns for downstream steps
        for col in ['Delta', 'Gamma', 'Vega', 'Theta', 'Bid', 'Ask', 'Mid', 'Open_Interest', 'Volume', 'Implied_Volatility']:
