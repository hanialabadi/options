...
def _calculate_dte_range_by_strategy(
    ...
    # DIRECTIONAL STRATEGIES (Long Call, Long Put)
        ...
        if confidence >= 75:
            return (20, 50, 'Short', 
                   f"Directional ({strategy_name}) with high confidence ({confidence}): 20-50 DTE for theta efficiency")
        else:
            return (25, 60, 'Short-Medium', 
                   f"Directional ({strategy_name}) with moderate confidence ({confidence}): 25-60 DTE for directional exposure")
    
    # VOLATILITY STRATEGIES (Straddle, Strangle — Step 6 emits without "Long" prefix)
    elif strategy_name in ['Long Straddle', 'Long Strangle', 'Straddle', 'Strangle']:
        # Relaxed DTE for testing: 30-90 DTE
        if iv_rank >= 70:
            return (30, 60, 'Short-Medium', 
                   f"Volatility ({strategy_name}) with elevated IV ({iv_rank:.0f}): 30-60 DTE for vol contraction play")
        elif iv_rank <= 30:
            return (60, 90, 'Medium', 
                   f"Volatility ({strategy_name}) with depressed IV ({iv_rank:.0f}): 60-90 DTE for vol expansion")
        else:
            return (45, 75, 'Medium', 
    ...
    # CREDIT SPREADS (Iron Condor, Put Credit Spread, Call Credit Spread)
        ...
        # Relaxed DTE for testing: 25-70 DTE
        if confidence >= 75:
            return (25, 50, 'Short', 
                   f"Credit strategy ({strategy_name}) with high confidence ({confidence}): 25-50 DTE for theta decay")
        else:
            return (40, 70, 'Medium', 
                   f"Credit strategy ({strategy_name}) with moderate confidence ({confidence}): 40-70 DTE for adjustment time")
    
    # DEBIT SPREADS (Bull Call, Bear Put, etc.)
    elif any(kw in strategy_name for kw in ['Debit Spread', 'Bull', 'Bear']):
        # Relaxed DTE for testing: 25-65 DTE
        return (25, 65, 'Short-Medium', 
               f"Debit spread ({strategy_name}): 25-65 DTE for directional play")
    
    # CALENDAR/DIAGONAL SPREADS
    elif any(kw in strategy_name for kw in ['Calendar', 'Diagonal']):
        # Relaxed DTE for testing: 40-90 DTE
        return (40, 90, 'Medium', 
               f"Calendar/Diagonal ({strategy_name}): 40-90 DTE for time decay differential")
    
    # FALLBACK BY STRATEGY TYPE
    elif strategy_type == 'Directional':
        return (25, 60, 'Short-Medium', 
               f"Directional strategy type: 25-60 DTE (generic directional)")
    elif strategy_type == 'Volatility':
        return (40, 80, 'Medium', 
               f"Volatility strategy type: 40-80 DTE (generic volatility)")
    elif strategy_type == 'Income':
        return (25, 55, 'Short', 
               f"Income strategy type: 25-55 DTE (generic income)")
    elif strategy_type in ['Mixed', 'Neutral']:
        return (30, 70, 'Medium', 
               f"{strategy_type} strategy type: 30-70 DTE (balanced Greeks)")
    
    # ULTIMATE FALLBACK
    else:
        logger.warning(f"⚠️ Unknown strategy '{strategy_name}' (type: {strategy_type}), using default DTE")
        return (30, 70, 'Medium', 
               f"Default timeframe for {strategy_name}: 30-70 DTE")
