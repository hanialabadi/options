...
def filter_ivhv_gap(df: pd.DataFrame, min_gap: float = 2.0) -> pd.DataFrame:
    ...
    Notes:
        ...
        - Leverages Surface_Shape, IV_Trend_7D, HV_Trend_30D from Step 2
        - Deduplication ensures 1 row per ticker
        - Column naming avoids strategy bias
    """
    # Validate Step 2 enriched fields exist
    required_from_step2 = ['IV_Rank_30D', 'Surface_Shape', 'IV_Trend_7D', 'HV_Trend_30D']
    ...
    # GAP 2 FIX: Relative gap floor — Natenberg Ch.8: "At high IV levels, 2pt absolute gap
    # is noise; at low IV levels, 2pt gap is highly significant."
    # Include condition: IVHV_gap_abs >= min_gap (2.0) OR IVHV_gap_30D_pct >= 8.0%
    # This is ADDITIVE — more tickers can pass, never fewer.
    # High-IV tickers (HV=80%) at gap=2.1 abs = 2.6% relative (filtered unless >= 8% relative gate is met)
    # Low-IV tickers (HV=18%) at gap=2.1 abs = 11.7% relative (passes both gates — meaningful)
    relative_gate_pct = 8.0  # 8% relative divergence = meaningful mispricing (Natenberg Ch.8)
    relative_gate_ok = df['IVHV_gap_30D_pct'].notna() & (df['IVHV_gap_30D_pct'].abs() >= relative_gate_pct)

    # Log relative gate inclusions (tickers passing only via relative gate, not absolute)
    abs_gate_ok = df['IVHV_gap_abs'] >= min_gap
    relative_only_count = (relative_gate_ok & ~abs_gate_ok & df['IV30_Call'].notna()).sum()
    if relative_only_count > 0:
        logger.info(f"[GAP2] Relative gate added {relative_only_count} tickers "
                    f"(gap_pct ≥ {relative_gate_pct:.0f}% but gap_abs < {min_gap:.1f})")

    # IMP 3 FIX: IVHV_gap_10D — Natenberg Ch.8: front-month gamma requires front-month vol comparison.
    # Income strategies with DTE < 21 should compare IV vs 10-day HV, not 30-day.
    # HV_10_D_Cur is computed from 10-day window in Step 0.
    hv_10_col = 'HV_10_D_Cur' if 'HV_10_D_Cur' in df.columns else ('hv_10' if 'hv_10' in df.columns else None)
    if hv_10_col:
        df['IVHV_gap_10D'] = df['IV30_Call'] - pd.to_numeric(df[hv_10_col], errors='coerce')
        df['ShortTerm_Front_Rich'] = df['IVHV_gap_10D'] >= 3.5  # Front-month premium selling signal
    else:
        df['IVHV_gap_10D'] = np.nan
        df['ShortTerm_Front_Rich'] = False

    # EWMA gap — Sinclair 2020 Ch.3: EWMA (λ=0.94) reacts to vol spikes in 2-3 days vs
    # 10-20 day lag of rolling HV. For short-dated income strategies (DTE < 21), the EWMA
    # provides a more current baseline for the IV-HV gap assessment.
    # HV_EWMA is computed in Step 0 (calculate_hv_ewma).
    if 'HV_EWMA' in df.columns:
        df['IVHV_gap_EWMA'] = df['IV30_Call'] - pd.to_numeric(df['HV_EWMA'], errors='coerce')
        # ShortTerm_EWMA_Rich: IV > EWMA by ≥ 2.0 — premium selling context for weekly/short DTE
        df['ShortTerm_EWMA_Rich'] = df['IVHV_gap_EWMA'] >= 2.0
        # ShortTerm_EWMA_Cheap: EWMA > IV by ≥ 2.0 — vol expansion / buy premium context
        df['ShortTerm_EWMA_Cheap'] = df['IVHV_gap_EWMA'] <= -2.0
        _ewma_rich_ct = int(df['ShortTerm_EWMA_Rich'].sum())
        _ewma_cheap_ct = int(df['ShortTerm_EWMA_Cheap'].sum())
        logger.info(f"[EWMA] gap computed: {_ewma_rich_ct} EWMA-Rich, {_ewma_cheap_ct} EWMA-Cheap")
    else:
        df['IVHV_gap_EWMA'] = np.nan
        df['ShortTerm_EWMA_Rich'] = False
        df['ShortTerm_EWMA_Cheap'] = False
        logger.debug("[EWMA] HV_EWMA column not in DataFrame — skipping EWMA gap (run Step 0 first)")

    # Filter for minimum divergence threshold
    # STRATEGY-NEUTRAL: Filter by absolute magnitude OR relative gate
    ...
    df_filtered = df[(~has_iv_data) | (df['IVHV_gap_abs'] >= min_gap) | relative_gate_ok].copy()

    # GUARDRAIL 2: Persist Gap_Trigger_Type column so downstream steps can audit HOW each ticker
    # passed the gap gate (ABS=absolute only, REL=relative only, BOTH=both, NONE=no-IV passthrough).
    # Without this column, relative-gate inclusions are untraceable from the output CSV.
    _abs_ok = df_filtered['IVHV_gap_abs'] >= min_gap
    _rel_ok = df_filtered['IVHV_gap_30D_pct'].notna() & (df_filtered['IVHV_gap_30D_pct'].abs() >= relative_gate_pct)
    _has_iv = df_filtered['IV30_Call'].notna()
    df_filtered['Gap_Trigger_Type'] = 'NONE'  # default: passed because no IV data
    df_filtered.loc[_has_iv & _abs_ok & ~_rel_ok,  'Gap_Trigger_Type'] = 'ABS'
    df_filtered.loc[_has_iv & ~_abs_ok & _rel_ok,  'Gap_Trigger_Type'] = 'REL'
    df_filtered.loc[_has_iv & _abs_ok & _rel_ok,   'Gap_Trigger_Type'] = 'BOTH'
    _rel_count = (_has_iv & ~_abs_ok & _rel_ok).sum()
    if _rel_count > 0:
        logger.info(f"[GAP_TRIGGER] {_rel_count} tickers passed via REL gate only (Gap_Trigger_Type='REL') — review for high-HV noise")
    ...
    if all(col in df_filtered.columns for col in ['IV_Rank_30D', 'IV_Trend_7D', 'HV_Trend_30D']):
        df_filtered['MeanReversion_Setup'] = (
            (df_filtered['IV_Rank_30D'] > 75) &  # IV in top 25% of recent range (Sinclair Ch.2: use top quartile for premium-selling setups)
