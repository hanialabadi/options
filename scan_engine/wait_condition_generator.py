...
def generate_wait_conditions_for_gate(
    ...
    # R4.1: Thesis quality — structural signal conflicts on READY directional
    # Conditions are pre-generated by check_thesis_quality() and stored in
    # _thesis_wait_conditions on the row. Use those directly if available.
    elif "R4.1" in gate_code:
        _pre_conditions = row_context.get('_thesis_wait_conditions')
        if _pre_conditions and isinstance(_pre_conditions, list):
            conditions.extend(_pre_conditions)
        # Minimum: wait 1 session for signals to update
        conditions.append(_create_time_delay_condition(
            next_session=True,
            description="Wait for next scan to re-evaluate thesis signals"
        ))

    # R5.0: Theory compliance floor — directional theory score below 50
    # Conditions are pre-generated by the R5.0 gate in step12 and stored in
    # _theory_compliance_conditions on the row. Includes theory score target,
    # MACD alignment, and ADX trend confirmation.
    elif "R5.0" in gate_code:
        _pre_conditions = row_context.get('_theory_compliance_conditions')
        if _pre_conditions and isinstance(_pre_conditions, list):
            conditions.extend(_pre_conditions)
        # Wait at least 1 session for chart signals to update
        conditions.append(_create_time_delay_condition(
            next_session=True,
            description="Wait for next scan to re-evaluate theory compliance"
        ))
