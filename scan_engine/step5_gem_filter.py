"""
Step 5: GEM Filter - Validate Data Quality

GOVERNANCE RULE: Enforces Signal_Type and Regime existence.
- Asserts Signal_Type is non-null (must come from Step 2)
- Asserts Regime is non-null (must come from Step 2)
- Fails pipeline if these authoritative fields are missing

This step does NOT modify Signal_Type or Regime - it validates they exist.
...
def validate_data_quality(df: pd.DataFrame) -> pd.DataFrame:
    ...
    #
    # Coverage denominator = tickers that have IV history (not MISSING).
    # Newly-onboarded tickers (IV_Maturity_State == 'MISSING') have 0 days of
    # history by design — they should not count against coverage quality.
    # The gate protects against data fetch failures, not deliberate onboarding.

    # Use IV_30_D_Call (live chain) if available; fall back to iv_30d (IVEngine history)
    if 'IV_30_D_Call' in df_result.columns and df_result['IV_30_D_Call'].notna().any():
        iv_col = 'IV_30_D_Call'
    else:
        iv_col = 'iv_30d'

    # Exclude tickers with no IV history yet (newly onboarded — not a data failure)
    maturity_col = 'IV_Maturity_State'
    if maturity_col in df_result.columns:
        iv_eligible = df_result[df_result[maturity_col] != 'MISSING']
        missing_count = (df_result[maturity_col] == 'MISSING').sum()
    else:
        iv_eligible = df_result
        missing_count = 0

    iv_populated = iv_eligible[iv_col].notna().sum() if iv_col in iv_eligible.columns else 0
    eligible_total = len(iv_eligible)
    total_tickers = len(df_result)
    iv_coverage_pct = (iv_populated / eligible_total * 100) if eligible_total > 0 else 0

    if missing_count:
        logger.info(
            f"📊 IV Coverage: {iv_populated}/{eligible_total} eligible tickers have IV "
            f"({iv_coverage_pct:.1f}%) | {missing_count} newly-onboarded tickers excluded from gate"
        )
